const stylePrefixes = {
  mx: [
    'marginLeft',
    'marginRight',
  ],
  my: [
    'marginTop',
    'marginBottom',
  ],
  px: [
    'paddingLeft',
    'paddingRight',
  ],
  py: [
    'paddingTop',
    'paddingBottom',
  ],
  h: 'height',
  'max-h': 'maxHeight',
  'min-h': 'minHeight',
  w: 'width',
  'max-w': 'maxWidth',
  'min-w': 'minWidth',
  m: 'margin',
  mt: 'marginTop',
  mr: 'marginRight',
  mb: 'marginBottom',
  ml: 'marginLeft',
  p: 'padding',
  pt: 'paddingTop',
  pr: 'paddingRight',
  pb: 'paddingBottom',
  pl: 'paddingLeft',
  flex: 'flex',
  rounded: 'borderRadius',
  'rounded-tl': 'borderTopLeftRadius',
  'rounded-tr': 'borderTopRightRadius',
  'rounded-br': 'borderBottomRightRadius',
  'rounded-bl': 'borderBottomLeftRadius',
  'border-b': 'borderBottomWidth',
  'border-e': 'borderEndWidth',
  'border-l': 'borderLeftWidth',
  'border-r': 'borderRightWidth',
  'border-s': 'borderStartWidth',
  'border-t': 'borderTopWidth',
  'border-w': 'borderWidth',
  'border': 'borderColor',
  'border-color-b': 'borderBottomColor',
  'border-color-e': 'borderEndColor',
  'border-color-l': 'borderLeftColor',
  'border-color-r': 'borderRightColor',
  'border-color-s': 'borderStartColor',
  'border-color-t': 'borderTopColor',
  bg: 'backgroundColor',
  color: 'color',
  'font-size': 'fontSize',
  'flex-shrink': 'flexShrink',
  text: 'color',
  'flex-grow': 'flexGrow',
  'aspect': 'aspectRatio',
  'z': 'zIndex',
  'l': 'left',
  'r': 'right',
  't': 'top',
  'b': 'bottom',
};

export default stylePrefixes;